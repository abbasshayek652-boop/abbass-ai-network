...
def train_classifier(dataset: Dataset, target: str) -> SupervisedResult:
    ...
    if y and accuracy < 0.5:
        weights = [-weight for weight in weights]
        bias = -bias
        model = NaiveClassifier(columns, weights, bias)
        probs = model.predict_proba(dataset)
        preds = model.predict(dataset)
        accuracy = sum(1.0 for pred, actual in zip(preds, y) if pred == actual) / len(y)
        roc = sum(1.0 for prob, actual in zip(probs, y) if (prob > 0.5) == bool(actual)) / len(y)
